<div class="front_page_section front_page_section_team<?php
			$raiderspirit_scheme = raiderspirit_get_theme_option('front_page_team_scheme');
			if (!raiderspirit_is_inherit($raiderspirit_scheme)) echo ' scheme_'.esc_attr($raiderspirit_scheme);
			echo ' front_page_section_paddings_'.esc_attr(raiderspirit_get_theme_option('front_page_team_paddings'));
		?>"<?php
		$raiderspirit_css = '';
		$raiderspirit_bg_image = raiderspirit_get_theme_option('front_page_team_bg_image');
...
?>><?php
	// Add anchor
	$raiderspirit_anchor_icon = raiderspirit_get_theme_option('front_page_team_anchor_icon');	
	$raiderspirit_anchor_text = raiderspirit_get_theme_option('front_page_team_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_team_inner<?php
			if (raiderspirit_get_theme_option('front_page_team_fullheight'))
				echo ' raiderspirit-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$raiderspirit_css = '';
			$raiderspirit_bg_mask = raiderspirit_get_theme_option('front_page_team_bg_mask');
			$raiderspirit_bg_color = raiderspirit_get_theme_option('front_page_team_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$raiderspirit_caption = raiderspirit_get_theme_option('front_page_team_caption');
			if (!empty($raiderspirit_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo !empty($raiderspirit_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post($raiderspirit_caption); ?></h2><?php
			}
		
			// Description (text)
			$raiderspirit_description = raiderspirit_get_theme_option('front_page_team_description');
			if (!empty($raiderspirit_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo !empty($raiderspirit_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post(wpautop($raiderspirit_description)); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_team_output"><?php 
				if (is_active_sidebar('front_page_team_widgets')) {
					...
					if (!raiderspirit_exists_trx_addons())
						...
						raiderspirit_customizer_need_widgets_message('front_page_team_caption', 'ThemeREX Addons - Team');
				}
			?></div>
		</div>
	</div>
