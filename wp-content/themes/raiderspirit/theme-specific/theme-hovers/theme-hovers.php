...
if (!function_exists('raiderspirit_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'raiderspirit_hovers_theme_setup3', 3 );
	function raiderspirit_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		raiderspirit_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'raiderspirit'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'raiderspirit') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'raiderspirit'),
					'slide_left'	=> esc_html__('Slide from Left',	'raiderspirit'),
					'slide_right'	=> esc_html__('Slide from Right',	'raiderspirit'),
					'slide_top'		=> esc_html__('Slide from Top',		'raiderspirit'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'raiderspirit'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'raiderspirit'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'raiderspirit') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'raiderspirit'),
					'icon'	=> esc_html__('Icon',	'raiderspirit'),
					'icons'	=> esc_html__('Icons',	'raiderspirit'),
					'zoom'	=> esc_html__('Zoom',	'raiderspirit'),
					'fade'	=> esc_html__('Fade',	'raiderspirit'),
					'slide'	=> esc_html__('Slide',	'raiderspirit'),
					'pull'	=> esc_html__('Pull',	'raiderspirit'),
					'border'=> esc_html__('Border',	'raiderspirit')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('raiderspirit_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'raiderspirit_hovers_theme_setup9', 9 );
	function raiderspirit_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',			'raiderspirit_hovers_frontend_scripts', 1010 );
		add_filter( 'raiderspirit_filter_localize_script','raiderspirit_hovers_localize_script' );
		add_filter( 'raiderspirit_filter_merge_scripts',	'raiderspirit_hovers_merge_scripts' );
		add_filter( 'raiderspirit_filter_merge_styles',	'raiderspirit_hovers_merge_styles' );
		add_filter( 'raiderspirit_filter_merge_styles_responsive', 'raiderspirit_hovers_merge_styles_responsive' );
		add_filter( 'raiderspirit_filter_get_css', 		'raiderspirit_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'raiderspirit_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'raiderspirit_hovers_frontend_scripts', 1010 );
	function raiderspirit_hovers_frontend_scripts() {
		if ( raiderspirit_is_on(raiderspirit_get_theme_option('debug_mode')) && raiderspirit_get_file_dir('theme-specific/theme-hovers/theme-hovers.js')!='' )
			wp_enqueue_script( 'raiderspirit-hovers', raiderspirit_get_file_url('theme-specific/theme-hovers/theme-hovers.js'), array('jquery'), null, true );
	}
}

// Merge hover effects into single css
if (!function_exists('raiderspirit_hovers_merge_styles')) {
	//Handler of the add_filter( 'raiderspirit_filter_merge_styles', 'raiderspirit_hovers_merge_styles' );
	function raiderspirit_hovers_merge_styles($list) {
...
if (!function_exists('raiderspirit_hovers_merge_styles_responsive')) {
	//Handler of the add_filter( 'raiderspirit_filter_merge_styles_responsive', 'raiderspirit_hovers_merge_styles_responsive' );
	function raiderspirit_hovers_merge_styles_responsive($list) {
...
if (!function_exists('raiderspirit_hovers_localize_script')) {
	//Handler of the add_filter( 'raiderspirit_filter_localize_script','raiderspirit_hovers_localize_script' );
	function raiderspirit_hovers_localize_script($arr) {
		$arr['button_hover'] = raiderspirit_get_theme_option('button_hover');
		return $arr;
	}
}

// Merge hover effects into single js
if (!function_exists('raiderspirit_hovers_merge_scripts')) {
	//Handler of the add_filter( 'raiderspirit_filter_merge_scripts', 'raiderspirit_hovers_merge_scripts' );
	function raiderspirit_hovers_merge_scripts($list) {
...
if ( !function_exists('raiderspirit_hovers_add_icons') ) {
	function raiderspirit_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat'])) {
					raiderspirit_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? ($product->is_type( 'variable' ) ? esc_html__('Select options', 'raiderspirit') : esc_html__('Buy now', 'raiderspirit')) : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'raiderspirit');
						else
							esc_html_e('Details', 'raiderspirit');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" ><span class="icon-untitled-1"></span><span class="icon-subs_plus"></span><span class="icon-untitled-2"></span></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'raiderspirit_hovers_get_css' ) ) {
	//Handler of the add_filter( 'raiderspirit_filter_get_css', 'raiderspirit_hovers_get_css', 10, 4 );
	function raiderspirit_hovers_get_css($css, $colors, $fonts, $scheme='') {
