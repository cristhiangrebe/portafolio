...
if ( in_array(get_post_type(), array( 'post', 'attachment' ) ) ) {
	?><div class="blogger_image"><span class="post_cat"><?php echo wp_kses_data(raiderspirit_get_post_categories('')); ?></span><?php
}
	// Featured image
	trx_addons_get_template_part('templates/tpl.featured.php',
									...
									apply_filters('trx_addons_filter_args_featured', array(
														...
														'hover' => 'zoomin',
														'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size($args['columns'] > 2 ? 'related' : 'big'), 'blogger-default')
														), 'blogger-default')
								);

	// Post content
	?></div><div class="sc_blogger_item_content entry-content"><?php
		...
		if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) {
			?><div class="sc_blogger_item_excerpt">
				...
				// More button
				if ( $show_more ) {
					?><div class="sc_blogger_item_button sc_item_button"><a href="<?php echo esc_url($post_link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button sc_button_simple', 'sc_blogger', $args)); ?>"><?php esc_html_e('Read more', 'raiderspirit'); ?></a></div><?php
				}
