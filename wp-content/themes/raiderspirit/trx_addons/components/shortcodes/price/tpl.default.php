		...
		?>>
			...
			if (!empty($item['image'])) {
				...
				if (!empty($item['image'])) {
					...
					?><div class="sc_price_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e('Icon', 'raiderspirit'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
			...
			?><div class="sc_price_item_info"><?php
				...
				if (!empty($item['link']) && !empty($item['link_text'])) {
					?><a href="<?php echo esc_url($item['link']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_price_item_link sc_button', 'sc_price', $args, $item)); ?>" target="_blank"><span class="icon-right"></span><?php
