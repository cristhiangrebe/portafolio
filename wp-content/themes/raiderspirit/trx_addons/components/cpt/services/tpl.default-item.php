...
?>>
	...
	if ($args['featured'] != 'none') {
		if ( has_post_thumbnail() && $args['featured']=='image') {
			...
			if (trx_addons_is_url($meta['icon'])) {
				...
				$svg_present = true;
			?><div class="icon_block"><a href="<?php echo esc_url($link); ?>"
	...
	<div class="sc_services_item_info">
		...
		<?php if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) { ?>
			...
			<div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'service_btn_simple', 'sc_services', $args)); ?>"><?php esc_html_e('Aprender más', 'raiderspirit'); ?></a></div>
		<?php } ?>
	</div>
</div>
<?php
